...
import time
import pandas as pd
from models.crnn_simple_mimic import cRNN_simple, train_model_adam
...
log = 'mimic_rnn_simple.log'
MODEL_NAME = 'crnnmimic_simple.pt'
BARCH_SIZE = 40000
H = 300
TRAIN = 'adam'
...
with open('mimic_rnn.log', "a") as logfile:
    logfile.write('MIMIC_CRNN_SIMPLE adam' + '  sum  '+ str(sum_list)+  '\n')
    logfile.write('MIMIC_CRNN_SIMPLE adam' + '  weight  '+  str(print_list_eval(class_weights))+  '\n')
crnn = cRNN_simple(x_train.shape[-1], output=y_train.shape[-1],hidden=H, T=x_train.shape[1], class_weights = class_weights).cuda(device=device)
...
with open('mimic_rnn.log', "a") as logfile:
  if TRAIN == 'adam':
    logfile.write('MIMIC_CRNN_SIMPLE adam  H '+ str(H) + '   time '+ str(end)+  '\n')
