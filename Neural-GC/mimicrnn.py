...
for i in range(output_num):
    ...
    weight_tmp = torch.tensor([1, 1]).float().cuda(device=device)
    print(weight_tmp)
...
train_loss_list = train_model_adam(
        crnn, X=x_train, Y=y_train , context=10, lam=0, lam_ridge=0, lr=1e-3, max_iter=20000,check_every=100, verbose=1)
